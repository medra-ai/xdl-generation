...
optional_properties = {
    ...
    "StartResuspend": ["vessel", "resuspend_speed", "purpose"],
    "Resuspend": [
        "vessel",
        "time",
        "resuspend_speed",
            ...
            if available_hardware:
                if component.attrib["id"] not in available_hardware:
                    ...
                    step_str = ET.tostring(
                        component, encoding="unicode", method="xml"
                    ).strip()
                    error_list.append(
...
def parse_reagents(root, error_list, available_reagents):
    ...
    for reagents in root.iter("Reagents"):
        for reagent in reagents.iter("Reagent"):
            ...
            if errors:
                step_str = ET.tostring(
                    reagent, encoding="unicode", method="xml"
                ).strip()
    ...
    for procedure in root.iter("Procedure"):
        for step in procedure:
            ...
            if action not in mandatory_properties:
                ...
                for attr in step.attrib:
                    if attr not in optional_properties[action]:
                        allowed_actions = list(
                            set(
                                optional_properties[action]
                                + mandatory_properties[action]
                            )
                        )
                        errors.append(
                ...
                if (
                    len(error_list) == 0
                    or "Hardware" not in error_list[0]["step"].lower()
                ):
...
def verify_xdl(xdl, available_hardware=None, available_reagents=None):
    ...
    try:
        ...
        return [
            {
                "errors": [
                    "Input XDL cannot be parsed as XML, there is {} error".format(
                        str(e).split(":")[0]
                    )
                ]
            }
